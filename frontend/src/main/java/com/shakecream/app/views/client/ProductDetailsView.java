...
import com.shakecream.app.models.Adicional;
...
public class ProductDetailsView {
    ...
    private final List<Adicional> todosAdicionais = new ArrayList<>();
    ...
    public void show(Stage stage, String nomeProduto, double precoBase, String pathImagem, String descricaoProduto, String categoriaSabor) {
        this.ehBebida = false;
    ...
    public void show(Stage stage, String nomeProduto, double precoBase, String pathImagem, String descricaoProduto, boolean isBebida) {
    ...
    private void renderizarTela(Stage stage, String nomeProduto, double precoBase, String pathImagem, String descricaoProduto) {
        ...
        imgWrapper.setStyle("-fx-background-color: white; -fx-background-radius: 20; -fx-border-color: #E0DDD9; -fx-border-radius: 20; -fx-border-width: 1;");

        ImageView imgView = new ImageView();
        try {
            imgView.setImage(new Image(getClass().getResourceAsStream("/com/shakecream/app/" + pathImagem)));
        ...
        counterBox.setStyle("-fx-background-color: white; -fx-background-radius: 12; -fx-border-color: #E0DDD9; -fx-border-radius: 12; -fx-padding: 8 15;");
        counterBox.setMaxWidth(180);

        Button btnMinus = new Button("-");
        btnMinus.setStyle("-fx-background-color: transparent; -fx-font-family: 'Montserrat'; -fx-font-size: 22; -fx-font-weight: bold; -fx-text-fill: " + COLOR_TEXT_DARK + "; -fx-cursor: hand; -fx-min-width: 35;");
        btnMinus.setOnAction(e -> { if (quantidade > 1) { quantidade--; atualizarValores(); } });

        lbQuantidade = new Label("1");
        lbQuantidade.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 22; -fx-font-weight: bold; -fx-text-fill: " + COLOR_TEXT_DARK + ";");

        Button btnPlus = new Button("+");
        btnPlus.setStyle("-fx-background-color: transparent; -fx-font-family: 'Montserrat'; -fx-font-size: 22; -fx-font-weight: bold; -fx-text-fill: " + COLOR_TEXT_DARK + "; -fx-cursor: hand; -fx-min-width: 35;");
        btnPlus.setOnAction(e -> { quantidade++; atualizarValores(); });

        counterBox.getChildren().addAll(btnMinus, lbQuantidade, btnPlus);

        lbPrecoTotal = new Label("R$ 0,00");
        lbPrecoTotal.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 36; -fx-font-weight: bold; -fx-text-fill: " + COLOR_PRIMARY + ";");

        Button btnAdicionarAoCarrinho = new Button("ADICIONAR");
        btnAdicionarAoCarrinho.setPrefSize(280, 55);
        btnAdicionarAoCarrinho.setStyle("-fx-background-color: " + COLOR_PRIMARY + "; -fx-text-fill: white; -fx-font-family: 'Montserrat'; -fx-font-weight: bold; -fx-font-size: 16; -fx-background-radius: 14; -fx-cursor: hand;");
        ...
        lbNome.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 32; -fx-font-weight: bold; -fx-text-fill: " + COLOR_TEXT_DARK + ";");

        Label lbDescricao = new Label(descricaoProduto);
        lbDescricao.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 15; -fx-text-fill: " + COLOR_TEXT_MUTED + ";");

        Label lbPrecoBase = new Label("R$ " + String.format("%.2f", precoProdutoBase));
        lbPrecoBase.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 22; -fx-font-weight: bold; -fx-text-fill: " + COLOR_PRIMARY + ";");

        colDireita.getChildren().addAll(lbNome, lbDescricao, lbPrecoBase);

        if (!ehBebida) {
            Label lbTituloTamanho = new Label("ESCOLHA O TAMANHO *");
            lbTituloTamanho.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 13; -fx-font-weight: bold; -fx-text-fill: " + COLOR_TEXT_MUTED + ";");
            ...
            lbTituloAdicionais.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 13; -fx-font-weight: bold; -fx-text-fill: " + COLOR_TEXT_MUTED + ";");
            ...
            lbTituloObs.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 13; -fx-font-weight: bold; -fx-text-fill: " + COLOR_TEXT_MUTED + ";");
            ...
            colDireita.getChildren().addAll(lbTituloTamanho, boxTamanhosLayout, lbTituloAdicionais, flowAdicionaisRapidos, lbTituloObs, txtObs);
        }
    ...
    private VBox renderSizeCard(String label, String ml, double valorExtra) {
        ...
        Label l1 = new Label(label); l1.setStyle("-fx-font-family: 'Montserrat'; -fx-font-weight: bold; -fx-font-size: 18;");
        Label l2 = new Label(ml); l2.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 12; -fx-text-fill: " + COLOR_TEXT_MUTED + ";");
        Label l3 = new Label("R$ " + String.format("%.2f", valorExtra)); l3.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 12; -fx-text-fill: " + COLOR_TEXT_DARK + ";");
        ...
        box.setOnMouseClicked(e -> {
            ...
            box.setStyle("-fx-background-color: #F8D7DA; -fx-border-color: " + COLOR_PRIMARY + "; -fx-border-radius: 15; -fx-background-radius: 15;");
            atualizarValores();
        });
    ...
    private void renderAdicionaisFigmaMenu() {
        ...
        if (!todosAdicionais.isEmpty()) {
            if (todosAdicionais.size() > 0) flowAdicionaisRapidos.getChildren().add(createAdicionalHorizontalCard(todosAdicionais.get(0)));
            if (todosAdicionais.size() > 1) flowAdicionaisRapidos.getChildren().add(createAdicionalHorizontalCard(todosAdicionais.get(1)));
        }
        ...
        cardMais.setStyle("-fx-background-color: white; -fx-background-radius: 15; -fx-border-color: #E0DDD9; -fx-border-radius: 15;");

        Label lbMais = new Label("+ MAIS");
        lbMais.setStyle("-fx-font-family: 'Montserrat'; -fx-font-weight: bold; -fx-font-size: 14; -fx-text-fill: " + COLOR_TEXT_DARK + ";");
    ...
    private StackPane createAdicionalHorizontalCard(Adicional ad) {
        StackPane container = new StackPane();
        VBox box = new VBox(3);
        box.setAlignment(Pos.CENTER);
        box.setPrefSize(125, 75);
        box.setStyle(ad.isSelecionado() ?
                "-fx-background-color: #F8D7DA; -fx-border-color: " + COLOR_PRIMARY + "; -fx-background-radius: 15; -fx-border-radius: 15;" :
                "-fx-background-color: white; -fx-border-color: #E0DDD9; -fx-background-radius: 15; -fx-border-radius: 15;");

        Label l1 = new Label(ad.getNome().toUpperCase()); l1.setStyle("-fx-font-family: 'Montserrat'; -fx-font-weight: bold; -fx-font-size: 11;");
        Label l2 = new Label("R$ " + String.format("%.2f", ad.getPreco())); l2.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 11; -fx-text-fill: " + COLOR_TEXT_MUTED + ";");
    ...
    private void abrirMenuScrollVerticalAdicionais() {
        ...
        titleModal.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 20; -fx-font-weight: bold; -fx-text-fill: " + COLOR_TEXT_DARK + ";");

        VBox listContent = new VBox(10);
        for (Adicional ad : todosAdicionais) {
            ...
            Label name = new Label(ad.getNome()); name.setStyle("-fx-font-family: 'Montserrat'; -fx-font-size: 14; -fx-font-weight: bold;");
            Region sp = new Region(); HBox.setHgrow(sp, Priority.ALWAYS);
            Label pr = new Label("+ R$ " + String.format("%.2f", ad.getPreco())); pr.setStyle("-fx-font-family: 'Montserrat'; -fx-text-fill: " + COLOR_PRIMARY + ";");
        ...
        btnSalvarModal.setStyle("-fx-background-color: " + COLOR_PRIMARY + "; -fx-text-fill: white; -fx-font-family: 'Montserrat'; -fx-font-weight: bold; -fx-background-radius: 10; -fx-cursor: hand;");
    ...
    private void atualizarValores() {
        ...
        double somaAdicionais = todosAdicionais.stream().filter(Adicional::isSelecionado).mapToDouble(Adicional::getPreco).sum();
    ...
    private void processarESalvarPedido(Stage stage, String nomeProduto) {
        if (!ehBebida) {
            if (tamanhoSelecionado.isEmpty()) {
                exibirAlertaAviso("Tamanho Obrigatório", "Por favor, selecione um tamanho (P, M ou G) antes de prosseguir.");
                return;
            }
        }

        List<Adicional> extrasSalvos = todosAdicionais.stream().filter(Adicional::isSelecionado).collect(Collectors.toList());
        double extraTamCalculo = (acrescimoTamanho == -1.0) ? 0.00 : acrescimoTamanho;
        double precoFinalCalculado = (precoProdutoBase + extraTamCalculo + extrasSalvos.stream().mapToDouble(Adicional::getPreco).sum()) * quantidade;

        String observacaoSalva = (txtObs != null) ? txtObs.getText() : "";

        ItemCarrinho novoPedido = new ItemCarrinho(nomeProduto, tamanhoSelecionado, extrasSalvos, observacaoSalva, quantidade, precoFinalCalculado, pathImagem);
    ...
    private void inicializarBancoAdicionais() {
        if (todosAdicionais.isEmpty()) {
            try {
                List<Adicional> adicionaisDoBanco = additionalService.getAll();
